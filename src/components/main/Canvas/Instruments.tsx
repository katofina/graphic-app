...
const Instruments = () => {
  ...
  function back() {
    ...
    if (fabric!.allDo.length > 0) {
      ...
      dispatch(
        canvas.actions.pushCancelDo(
          (fabric.canvas as HTMLCanvasElement).toDataURL(),
        ),
      );
      let img = new Image();
      ...
      img.onload = () => {
        context!.clearRect(
          0,
          0,
          (fabric.canvas as HTMLCanvasElement).width,
          (fabric.canvas as HTMLCanvasElement).height,
        );
        context!.drawImage(
          img,
          0,
  ...
  function save() {
    if (!isLoggedIn()) {
      ...
      const same = fabric.save.some(
        (el) => el === (fabric.canvas as HTMLCanvasElement).toDataURL(),
      );
      if (!same)
        dispatch(
          canvas.actions.pushSave(
            (fabric.canvas as HTMLCanvasElement).toDataURL("image/png"),
          ),
        );
    }
  }
  ...
  return (
    <div className="panel-instrument">
      <button
        ...
        onClick={() =>
          dispatch(option.actions.setOption(new Brush(fabric.canvas, tools)))
        }
      ></button>
      <button
        className="instrument-button square"
        onClick={() =>
          dispatch(option.actions.setOption(new Square(fabric.canvas, tools)))
        }
      ></button>
      <button
        className="instrument-button circle"
        onClick={() =>
          dispatch(option.actions.setOption(new Circle(fabric.canvas, tools)))
        }
      ></button>
      <button
        className="instrument-button rubber"
        onClick={() =>
          dispatch(option.actions.setOption(new Rubber(fabric.canvas, tools)))
        }
      ></button>
      <button
        className="instrument-button line"
        onClick={() =>
          dispatch(option.actions.setOption(new Line(fabric.canvas, tools)))
        }
      ></button>
      <input
        type="color"
